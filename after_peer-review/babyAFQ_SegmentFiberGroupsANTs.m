function [fg_classified,fg_unclassified,classification,fg] = ...
    babyAFQ_SegmentFiberGroupsANTs(dt6File, fg, Atlas, ...
    useRoiBasedApproach, useInterhemisphericSplit, anatFile)

%%%%%%%%%%%%%%%%%%%%
% This is an infant implementation of AFQ_SegmentFiberGroups
% The documentation for the original function in enclosed below

% In brief, the implemented infant-specfic changes are:
% 1) An infant specfic templant (UNCNeo) is used as a common references
% space instead of the MNI brian
% 2) Waypoint ROIs were adjusted for the infant brain. Adjustments include
% spatially more restrcitive ROIs as well as the addition of a third ROI
% for some bundles
% 3) The mdLF is identified as an additional bundle not included in
% adultAFQ
% 4) The probabilistic JHU atlas was warped into the infant template space
% 5) The infant way-point ROIs are saved in a folder called babyAFQ, to
% avoid confusion
% 6) Users have the option to supply a T2 weithed anatomical image
% (anatFile) for alignment. If no image is supplied, the default b0 image
% will be used
% 7) pAF and VOF ROIs are tranformed to indivudal subject space in
% preparation for babyAFQ_FindVOF
...
if(~exist('Atlas','var') || isempty(Atlas))
    %Default scenario: use baby Mori Atlas
    Atlas='UNCNeo_JHU_tracts_prob-for-babyAFQ.nii.gz';
end
...
tdir = fullfile(AFQbase,'babyAFQ/templates');
template = fullfile(tdir,'UNCNeo-withCerebellum-for-babyAFQ.nii.gz');
...
% Rescale image valueds to get bettergray/white/CSF contrast
...
% alignment
% imwrite(makeMontage(im),fullfile(baseDir, 'babyAFQ_SpatialNormalization.png'));
...
%for mdLF
moriTracts.data(:,:,:,21) = moriTracts.data(:,:,:,13);
moriTracts.data(:,:,:,22) = moriTracts.data(:,:,:,14);
moriTracts.dim = [91,109,91,22];
...
%--------------------------------------------------------------------------------
% TKT (2022) added below from ANTS normalization in earlier step of AFQ_run_infant
%--------------------------------------------------------------------------------

% normalize with ANTs
[panat,~] = fileparts(anatFile); % retrieve transformations from same directory as anatomical image
ANTS_normalize_baby(anatFile, template, panat); % make it automatic to brainmask.nii.gz infFS directory

% necessary elements for downstream ANTS implementation
% affineXform = [panat '/babyReg0GenericAffine.mat'];
invAffineXform = ['[' panat '/babyReg0GenericAffine.mat,1]'];
antsInvWarp = [panat '/babyReg1InverseWarp.nii.gz'];
fibin = [panat '/fibin.csv'];
fibout = [panat '/fibout.csv'];
b0 = fullfile(baseDir,'bin','b0.nii.gz'); % need for reference image for antsApplyTransforms

% send fiber coordinates to template space
fg_sn = ANTS_applyTransform2Points_baby(fg, antsInvWarp, invAffineXform, fibin, fibout); 
save([panat '/xformFibs.mat'],'fg_sn');
...
save([panat '/xformFibsAtlas.mat'],'fgCoords');
save([panat '/xformFibsAtlas2.mat'],'fgCoords2');
...
if useRoiBasedApproach
    % A 22x3 cell array containing the names of both waypoint ROIs for each
    % of 22 fiber groups
    moriRois={'ATR_roi1_L.nii.gz',  'ATR_roi2_L.nii.gz',  'ATR_roi3_L.nii.gz'; 'ATR_roi1_R.nii.gz', 'ATR_roi2_R.nii.gz', 'ATR_roi3_R.nii.gz'; ...
        ...
        'IFO_roi1_L.nii.gz', 'IFO_roi2_L.nii.gz', 'IFO_roi3_L.nii.gz'; 'IFO_roi1_R.nii.gz', 'IFO_roi2_R.nii.gz', 'IFO_roi3_R.nii.gz'; ...
        ...
        'UNC_roi1_L.nii.gz', 'UNC_roi2_L.nii.gz', 'UNC_roi3_L.nii.gz'; 'UNC_roi1_R.nii.gz', 'UNC_roi2_R.nii.gz', 'UNC_roi3_R.nii.gz'; ...
        'SLF_roi1_L.nii.gz', 'SLFt_roi2_L.nii.gz', 'SLFt_roi3_L.nii.gz'; 'SLF_roi1_R.nii.gz', 'SLFt_roi2_R.nii.gz', 'SLFt_roi3_R.nii.gz'; ...
        'MdLF_roi1_L.nii.gz', 'ILF_roi2_L.nii.gz', ' '; 'MdLF_roi1_R.nii.gz', 'ILF_roi2_R.nii.gz', ' '};
    ...
    keep1=zeros(length(fg.fibers), size(moriRois, 1)); keep2=zeros(length(fg.fibers), size(moriRois, 1));  keep4=zeros(length(fg.fibers), size(moriRois, 1));
    ...
    for roiID=1:size(moriRois, 1)
        ...
        ROI_img_file=fullfile(tdir, 'UNC_Neo_tracts_ROIs',  [moriRois{roiID, 1}]);
        ...
        if recomputeROIs
            ...
            ANTS_CreateRoiFromMniNifti_baby(ROI_img_file, b0, invAffineXform, antsInvWarp, outfile); % TKT 2022.03.28 made refNifti = b0

            % Convert nifti image to .mat roi
            roi1(roiID) = dtiRoiFromNifti(outfile, [], prefix(prefix(outfile)), 'mat');

            % Clean the ROI
            roi1(roiID) = dtiRoiClean(roi1(roiID), 3, {'fillHoles'});

            % Save the ROI
            dtiWriteRoi(roi1(roiID), prefix(prefix(outfile)));           
        ...
        ROI_img_file=fullfile(tdir,  'UNC_Neo_tracts_ROIs',  [moriRois{roiID, 2}]);
        ...
        if recomputeROIs
            ...
            ANTS_CreateRoiFromMniNifti_baby(ROI_img_file, b0, invAffineXform, antsInvWarp, outfile);

            % Convert nifti image to .mat roi
            roi2(roiID) = dtiRoiFromNifti(outfile, [], prefix(prefix(outfile)), 'mat');

            % Clean the ROI
            roi2(roiID) = dtiRoiClean(roi2(roiID), 3, {'fillHoles'});

            % Save the ROI
            dtiWriteRoi(roi2(roiID), prefix(prefix(outfile)));
        ...
        if roiID <3 || roiID==5 || roiID==6 || roiID==11 || roiID==12 || roiID==17 || roiID==18 || roiID==19 || roiID==20
            ...
            % Transform ROI-2 to an individuals native space
            if recomputeROIs
                ROI_img_file=fullfile(tdir, 'UNC_Neo_tracts_ROIs',  moriRois{roiID, 3});
                    
                % Default is to use the spm normalization unless a superior
                % ANTS normalization was passed in
                rDir = fullfile(fileparts(dt6File),'ROIs');
                if ~exist(rDir,'dir'); mkdir(rDir); end
                outfile = fullfile(fileparts(dt6File), 'ROIs',moriRois{roiID, 3});
                ANTS_CreateRoiFromMniNifti_baby(ROI_img_file, b0, invAffineXform, antsInvWarp, outfile);

                % Convert nifti image to .mat roi
                roi3(roiID) = dtiRoiFromNifti(outfile, [], prefix(prefix(outfile)), 'mat');

                % Clean the ROI
                roi3(roiID) = dtiRoiClean(roi3(roiID), 3, {'fillHoles'});

                % Save the ROI
                dtiWriteRoi(roi3(roiID), prefix(prefix(outfile)));

            else
                RoiFileName=fullfile(fileparts(dt6File), 'ROIs',  [prefix(prefix(ROI_img_file, 'short'), 'short') '.mat']);
                roi3(roiID) = dtiReadRoi(RoiFileName);
            end
            
            keepID2=find(keep2(:, roiID));
            fgCopy.fibers=fg.fibers(keepID2(keepID2>0));
            [a,b, keep4given2] = dtiIntersectFibersWithRoi([], 'and', minDist, roi3(roiID), fgCopy);
            keep4(keepID2(keep4given2), roiID)=true;
            keepID4=find(keep4(:, roiID));
            keep2(:, roiID)=keep4(:,roiID);
        end
    end
    
    %% In babyAFQ we prepare the 3D ROIs for the VOF and pAF already here
    if recomputeROIs
        ROI_img_file=fullfile(tdir,'UNC_Neo_tracts_ROIs', 'VOF_box_L.nii.gz');
        outfile = fullfile(fileparts(dt6File), 'ROIs', 'VOF_box_L.nii.gz');
        ...
        ROI_img_file=fullfile(tdir,'UNC_Neo_tracts_ROIs', 'VOF_box_R.nii.gz');
        outfile = fullfile(fileparts(dt6File), 'ROIs', 'VOF_box_R.nii.gz');
        %[RoiFileName, invDef, roi1(roiID)]=dtiCreateRoiFromMniNifti(dt6File, ROI_img_file, invDef, true, [],[]);
        ANTS_CreateRoiFromMniNifti_baby(ROI_img_file, b0, invAffineXform, antsInvWarp, outfile);
        ...
        ROI_img_file=fullfile(tdir,'UNC_Neo_tracts_ROIs', 'LH_Parietal.nii.gz');
        outfile = fullfile(fileparts(dt6File), 'ROIs', 'LH_Parietal.nii.gz');
        % [RoiFileName, invDef, roi1(roiID)]=dtiCreateRoiFromMniNifti(dt6File, ROI_img_file, invDef, true, [],[]);
        ANTS_CreateRoiFromMniNifti_baby(ROI_img_file, b0, invAffineXform, antsInvWarp, outfile);
        ...
        ROI_img_file=fullfile(tdir,'UNC_Neo_tracts_ROIs', 'RH_Parietal.nii.gz');
        outfile = fullfile(fileparts(dt6File), 'ROIs', 'RH_Parietal.nii.gz');
        % [RoiFileName, invDef, roi1(roiID)]=dtiCreateRoiFromMniNifti(dt6File, ROI_img_file, invDef, true, [],[]);
        ANTS_CreateRoiFromMniNifti_baby(ROI_img_file, b0, invAffineXform, antsInvWarp, outfile);
...
% We have a set of atlas scores for each fiber. To categorize the
...
% rename ROI folder to babyAFQROIs to avoid confusion
cmd = ['rm -r ' fullfile(fileparts(dt6File),'babyAFQROIs')];
system(cmd);

cmd=['mv -f ' fullfile(fileparts(dt6File),'ROIs') ' ' fullfile(fileparts(dt6File),'babyAFQROIs')];
    system(cmd);
return;
